...
export class AnalyticsChartComponent {
  ...
  approvedHeight = computed(() => {
    const count = this.data().filter((enrollment) => enrollment.status === 'Approved').length;

    return Math.max(20, count * 3);
  });

  pendingHeight = computed(() => {
    const count = this.data().filter((enrollment) => enrollment.status === 'Pending').length;

    return Math.max(20, count * 3);
  });

  rejectedHeight = computed(() => {
    const count = this.data().filter((enrollment) => enrollment.status === 'Rejected').length;
